...
#define BLE_UUID_GYRO_SENSOR_DATA "4664E7A1-5A13-BFFF-4636-7D0A4B16496D"
...
union multi_sensor_data gyroSensorData;
...
BLECharacteristic gyroSensorDataCharacteristic(BLE_UUID_GYRO_SENSOR_DATA, BLERead | BLENotify, sizeof gyroSensorData.bytes);
...
void setup() {
  ...
  for (int i = 0; i < NUMBER_OF_SENSORS; i++) {
    ...
    gyroSensorData.values[i] = 0.0;
  }
}

void loop() {
  float x, y, z, gx, gy, gz;
#define UPDATE_INTERVALL 400
      ...
      if (currentMillis - previousMillis > UPDATE_INTERVALL) {
        ...
        if (central.rssi() != 0) {
          ...
          if (IMU.gyroscopeAvailable()) {
            ...
            gyroSensorData.values[0] = gx;
            gyroSensorData.values[1] = gy;
            gyroSensorData.values[2] = gz;
            Serial.println("hoi");
            Serial.println(gyroSensorData.values[0]);
          }

          accelSensorDataCharacteristic.writeValue(accelSensorData.bytes, sizeof accelSensorData.bytes);
          gyroSensorDataCharacteristic.writeValue(gyroSensorData.bytes, sizeof gyroSensorData.bytes);
        }
      }
    }
...
bool setupBleMode() {
  ...
  BLE.setDeviceName("BLE-LAB55");
  BLE.setLocalName("BLE-LAB55");
  BLE.setAdvertisedService(sensorDataService);

  // BLE add characteristics
  sensorDataService.addCharacteristic(accelSensorDataCharacteristic);
  sensorDataService.addCharacteristic(gyroSensorDataCharacteristic);
